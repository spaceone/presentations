...
Content-Length: 1738
Content-Type: application/univention
...
for key, value in parse_ldif(ldif):
	dom.by_id('data').append('<p><span>%s:</span><span>%s</span></p>' % (escape(key), escape(value))
```
```
</python>
<data>
